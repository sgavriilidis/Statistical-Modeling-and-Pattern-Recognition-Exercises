...
class Dense():
    ...
    def forward(self, input):
        self.input = input
        # Add Code Here
        fwd = np.multiply(self.weights,self.input)+ self.bias
        return fwd
    ...
    def backward(self, dE_dY, learning_rate):
        ...
        dE_dW = 1./m * np.dot(dE_dY,self.input.T)  
        dE_dX =np.dot(self.weights.T,dE_dY)
        dE_dB =(1/m)*np.sum(dE_dY, axis=1, keepdims=True)
    ...
    def update_weights(self, dE_dW, dE_dB, learning_rate):
        ...
        self.bias = self.weights - learning_rate*dE_dB
...
def loss_cross_entropy(y_true, y_pred):
    # Add Code Here
    m = y_true.shape[1]
    L = -(1./m) * ( np.sum( np.multiply(np.log(y_pred),y_true) ) + np.sum( np.multiply(np.log(1-y_pred),(1-y_true)) ) )
    return loss
