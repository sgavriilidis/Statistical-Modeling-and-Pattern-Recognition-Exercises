...
% Get the data for each class
reduced_IRIS1  =  meas_reduced(iris_labels == 0,:);   %Samples of Class 0
reduced_IRIS2  =  meas_reduced(iris_labels == 1,:);	 %Samples of Class 1
reduced_IRIS3  =  meas_reduced(iris_labels == 2,:);	 %Samples of Class 2

%  Visualize the sample dataset after LDA is applied
%  Use different color/symbol for each class
figure(4)
hold on
plot(reduced_IRIS1(:, 1), reduced_IRIS1(:, 2), 'bo');
plot(reduced_IRIS2(:, 1), reduced_IRIS2(:, 2), 'rs');
plot(reduced_IRIS3(:, 1), reduced_IRIS3(:, 2), 'g+');
hold off
