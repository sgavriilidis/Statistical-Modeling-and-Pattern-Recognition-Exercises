...
[eigenvaluesSorted, order] = sort (diag(S), 'descend'); % sort ->diag : vector of eigenvalues
eigvectors = U(:, order);
PCvariance = eigenvaluesSorted/sum(eigenvaluesSorted) ;
...
K = 1;
Z = projectData(X_norm, eigvectors, K);
...
X_rec  = recoverData(Z, eigvectors, K);
